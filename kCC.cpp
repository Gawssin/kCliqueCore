	...
	while (true)
	{
		...
		for (int i = 0; i < g->n; i++)
		{
			if (label[i] != -1)
			{
				...
				{
					int u = g->adj[j];
					//if (col[u] < 0 || col[u] >= tolcol)
					//{
					//	cout << "overflow: " << col[u] << " " << tolcol  << endl;
					//	return;
					//}
...
void peeling(Graph *g, int k, double lwb)
{
	...
	int *delArr = new int[g->n], delN = 0;
	int gtlwb = 0;
	for (int i = 0; i < g->n; i++)
		if (label[i] != -1 && cnt[i]*1.0 < lwb)
		{
			delArr[gtlwb++] = i;
		}

	for (int i = 0; i < subN; i++) cnt[subG[i]] = 0;

	//cout << "gtlwb:" << gtlwb << endl;

	deleteNodes(g, delArr, gtlwb);

	tol = 0; subN = 0;
	//memset(cnt, 0, sizeof(long long) * g->n);
	...
	tolCliques = tol;
	cout << "gtlwb:" << gtlwb << " lessN: " << lessN << endl;

	high_resolution_clock::time_point t1 = high_resolution_clock::now();
	auto t01 = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();

	cout << "t0->t1 time: " << t01 / 1e6 << endl;
	//*********************
